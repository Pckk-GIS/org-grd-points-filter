...
def test_process_skips_file_id_pairs_whose_grd_bounds_do_not_intersect_regions(
    ...
    assert report.output_counts["org"]["1"] == 1
    assert report.output_counts["grd"]["1"] == 1
    assert (config.output_dir / "org_region1.txt").read_text(
        encoding="utf-8"
    ).strip() == "1,5,5,100"
    assert (config.output_dir / "grd_region1.txt").read_text(
        encoding="utf-8"
    ).strip() == "1,6,6,200"
...
def test_process_uses_region_id_in_output_filename(tmp_path: Path) -> None:
    ...
    assert (config.output_dir / "org_region4.txt").read_text(
        encoding="utf-8"
    ).strip() == "1,5,5,100"
    assert (config.output_dir / "org_region8.txt").read_text(encoding="utf-8") == ""
